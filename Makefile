...
.PHONY: docker
docker:
	docker run -it --entrypoint bash serverless-ec2scheduler-dev:appimage
...
stackstart: userdata.sh
	aws cloudformation create-stack --stack-name ec2scheduler --template-body file://ec2_instance.yaml \
	    --parameters ParameterKey=UserDataFile,ParameterValue=$(shell base64 -i userdata.sh | tr -d '\n') \
	    --capabilities CAPABILITY_NAMED_IAM
	aws cloudformation wait stack-create-complete --stack-name ec2scheduler
	# rm userdata.sh

.PHONY: stackinfo
stackinfo:
	aws cloudformation describe-stacks --stack-name ec2scheduler --query 'Stacks[*].Outputs'

.PHONY: stackstop
stackstop:
	aws cloudformation delete-stack --stack-name ec2scheduler
	aws cloudformation wait stack-delete-complete --stack-name ec2scheduler

.PHONY: logs_
logs_:
	$(eval INSTANCE_ID := $(shell aws cloudformation describe-stacks --stack-name ec2scheduler --query 'Stacks[*].Outputs[?OutputKey==`InstanceId`].OutputValue' --output text))
	@echo INSTANCE_ID=$(INSTANCE_ID)
	aws ec2 get-console-output --instance-id $(INSTANCE_ID)  --output text

.PHONY: ssh_key
ssh_key:
	INSTANCE_ID=$$(aws cloudformation describe-stacks --stack-name ec2scheduler --query 'Stacks[*].Outputs[?OutputKey==`InstanceId`].OutputValue' --output text) && \
	echo INSTANCE_ID=$${INSTANCE_ID} && \
...
stacklogs: connect_ssh_key
	PUBLIC_IP=$$(aws cloudformation describe-stacks --stack-name ec2scheduler --query 'Stacks[*].Outputs[?OutputKey==`PublicIP`].OutputValue' --output text) && \
